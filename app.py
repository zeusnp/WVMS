...
def get_party_names():
    ...
    # Query distinct party names that match the term
    parties = db.session.query(Vehicle.party_name)\
        .distinct()\
        .filter(Vehicle.party_name.ilike(f'%{term}%'))\
        .limit(10)\
        .all()
    return jsonify([party[0] for party in parties])

@app.route('/get_sub_party_names')
@login_required
def get_sub_party_names():
    term = request.args.get('term', '')
    party_name = request.args.get('party_name', '')
    # Query distinct sub-party names that match the term and party name
    query = db.session.query(Vehicle.sub_party_name)\
        .distinct()\
        .filter(Vehicle.sub_party_name.ilike(f'%{term}%'))
    
    if party_name:
        query = query.filter(Vehicle.party_name == party_name)
    
    sub_parties = query.limit(10).all()
    return jsonify([sub_party[0] for sub_party in sub_parties if sub_party[0]])

@app.route('/get_vehicle_numbers')
@login_required
def get_vehicle_numbers():
    term = request.args.get('term', '')
    # Query distinct vehicle numbers that match the term
    vehicles = db.session.query(Vehicle.vehicle_no)\
        .distinct()\
        .filter(Vehicle.vehicle_no.ilike(f'%{term}%'))\
        .limit(10)\
        .all()
    return jsonify([vehicle[0] for vehicle in vehicles])
